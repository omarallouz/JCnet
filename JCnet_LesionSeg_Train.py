...
from CNNUtils.TrainingMonitor import TrainingMonitor
import matplotlib
matplotlib.use('Agg')
...
def main(**kwargs):
    ...
    f = 0
    for i in train_indx:
        maskname = check_nifti_filepath(kwargs['atlasdir'], ('atlas%d' % (i + 1)) + '_' + 'lesion')
    ...
    print('Total number of training Lesion patches = ' + str(int(f)))
    loss = kwargs['loss']
    ...
    outname = 'JCnet_LesionSeg_Model_' + kwargs['model'].upper() + '_' + psize + '_Orient%d%d%d_' + con + '_' + time_id + '.h5'
    ...
    for orient in range(1 if kwargs['axial_only'] else 3):
        ...
        for i in range(kwargs['numatlas']):
            ...
            segpath = check_nifti_filepath(kwargs['atlasdir'], ('atlas%d' % (i + 1)) + '_' + 'lesion') # Original
        ...
        figPath= orient_outname.replace('.h5', '_loss-accuracy.png')
        jsonPath = orient_outname.replace('.h5', '_loss-accuracy.json')
        
        callbacks.append(dlr)
        callbacks.append(tensorboard)
        callbacks.append(TrainingMonitor(figPath , jsonPath=jsonPath))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='JCnet Lesion Segmentation Training')
    ...
    optional.add_argument('--loss', type=str, default='bce',
                        help="Loss function to be used during training. Available options are mae (mean absolute error), "
                             ...
                             "binary lesion masks are first blurred by a Gaussian to compute a membership. If model is "
    ...
    if str(results.model).lower() not in model:
        sys.exit('Error: Available training model options are Inception, Unet, Densenet, Vnet, or FPN/PanopticFPN.  '
